...
const log = named_log('quadbase stats')
...
const special_details = {
  ...
  speedy: `speedy could play as well as beast if they took a little`,
...
export default ({ user, hf, handle, profile_map, embedded=false }) => {
  ...
  useF(viewer, user, async () => {
    if (viewer && viewer !== user) {
      const { stats:pair_stats } = await api.get(`/quadbase/pair/${viewer}/${user}`)
      log({pair_stats})
  ...
  useM(user, () => historic_users.current.push(user))
    ...
    <InfoSection labels={[
      ...
      {edit ? null : profile ? <>
        <div className='middle-column wide'>
          ...
          })(stats?.letters_played ? Math.ceil((Date.now() - profile.t) / datetime.duration({ d:1 })) : 0)}
        </div>
      </> : <div>loading!</div>}
      {edit ? <>
        select color:
    ...
    {edit ? null : special_detail ? <>
      ...
      {profile ? <>
        ...
        {pair_stats ? <>
          ...
          <InfoSection labels={['total games']}>
            {pair_stats.total||0}
          </InfoSection>
                  ...
                  still working on the first game {pair_stats.total /*|| !viewer_friends_set.has(user)*/ ? null : <InfoBadges labels={[
                    {
                      text: 'new game',
                      func: async e => {
                        handle.open(await create_game([viewer, user]))
                      }
                    }
                  ]} />}
                </div>
