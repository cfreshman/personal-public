...
export const set_lang = (lang: string) => {
    console.debug(`QUADBASE LANG`, lang)
    dict.set({
        lang,
        loaded: false,
        words: new Set<string>(),
    ...
    return load_lang(lang).then(value => {
        ...
        dict.set(value)
        console.debug('QUADBASE LANG LOADED', dict.lang)
    })
}
...
export const load_lang = (lang: string='english'): Promise<any> => Promise.resolve(Object.load(dicts, lang, () => {
    ...
    const fetch_dict = () => {
        ...
        .then(res => {
            console.debug(`QUADBASE DICT FETCHED`, lang)
        ...
        .then(word_list => {
            console.debug(`QUADBASE DICT DESERIALIZED`, lang)
            ...
            dicts[lang] = _dict
            try {
                store.set(`quadbase-dict`, { ..._dict, words: [..._dict.words] })
            } catch {} // too large
            return _dict
        })
    ...
    const existing = store.get(`quadbase-dict`)
...
const freqsCalc = {}
target && dict.add(() => {
    console.debug('QUADBASE FREQ', target, dict)
    ...
    console.debug('QUADBASE FREQ', target, freqsCalc)
    if (Object.keys(freqsCalc).length === 2) {
        ...
        freqsCalc['alpha'] = letters
        {
            ...
            console.log('QUADBASE FREQ', target, freqsCalc)
        }
    }
})