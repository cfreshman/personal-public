...
export default () => {
  ...
  useEventListener(window, 'pointermove', e => {
    ...
    if (resize) {
      evt_stop(e)
      const rect = r.current.getBoundingClientRect()
      const [x_e, y_e] = [(e.clientX - rect.x) / rect.width, (e.clientY - rect.y) / rect.height]
      
      const h_sw = Q(l, '.handle-sw'), h_ne = Q(l, '.handle-ne')
      const rect_sw = h_sw.getBoundingClientRect(), rect_ne = h_ne.getBoundingClientRect()
      const [x_sw, y_sw] = [(rect_sw.x - rect.x) / rect.width, (rect_sw.y - rect.y) / rect.height]
      const [x_ne, y_ne] = [(rect_ne.x - rect.x) / rect.width, (rect_ne.y - rect.y) / rect.height]

      const [x_off, y_off] = [x_e - x_sw, -(y_e - y_sw)]
      const angle = Math.PI * data.angle / 180
      const [x_dim, y_dim] = [x_off * Math.cos(angle) - y_off * Math.sin(angle), x_off * Math.sin(angle) + y_off * Math.cos(angle)]

      const bottom_left = [data.x, data.y + data.h]
      const top_right = [data.x + x_dim, data.y + data.h - y_dim]

      // reset < 0 to original size
      if (top_right[1] > bottom_left[1]) {
        top_right[1] = data.y
      }
      if (top_right[0] < bottom_left[0]) {
        top_right[0] = data.x + data.w
      }

      l.style.top = `${(top_right[1]) * 100}%`
      l.style.left = `${(bottom_left[0]) * 100}%`
      l.style.width = `${(top_right[0] - bottom_left[0]) * 100}%`
      l.style.height = `${(bottom_left[1] - top_right[1]) * 100}%`

      text_id_to_resize[data.id]()
    }
    if (crop_ne) {
      evt_stop(e)
  ...
  useEventListener(window, 'pointerup pointercancel click', e => {
    ...
    if (resize) {
      evt_stop(e)
      const rect = r.current.getBoundingClientRect()
      const h_sw = Q(l, '.handle-sw')
      const rect_sw = h_sw.getBoundingClientRect()
      const [x_sw, y_sw] = [(rect_sw.x - rect.x) / rect.width, (rect_sw.y - rect.y) / rect.height]
      const [x_e, y_e] = [(e.clientX - rect.x) / rect.width, (e.clientY - rect.y) / rect.height]
      ...
      data.x = bottom_left[0]
      data.y = top_right[1]
      data.w = top_right[0] - bottom_left[0]
      data.h = bottom_left[1] - top_right[1]
      resize = undefined
      do_change = true
    }
    if (crop_ne) {
          ...
          {entities.map(({ id, type, data }) => {
            ...
            if (type === TYPE.IMAGE) {
              ...
              `)} onPointerDown={e => {
                ...
                <div className='control handle-sw' style={S(`
                position: absolute; bottom: 0; left: 0;
                pointer-events: none;
                `)} />
                <div className='control handle-ne' style={S(`
