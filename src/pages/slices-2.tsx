...
export default () => {
  ...
  const [play, set_play] = store.use('slices-2-play', { default:false })

  const [start_color, set_start_color] = store.use('slices-2-start-color', { default:undefined })
  const [end_color, set_end_color] = store.use('slices-2-end-color', { default:undefined })
  const [color_long, set_color_long] = store.use('slices-2-color-long', { default:false })

  const [back_color, set_back_color] = store.use('slices-2-back-color', { default:'#eeebe6' })
  ...
  useE(shape, play, reroll, start_color, end_color, color_long, () => {
  ...
  const shape_controls = [
    ...
    { [play ? 'dynamic' : 'static']: () => set_play(!play) },
  ]
  ...
  const back_controls = [
    ...
    back_color !== '#eeebe6' && { 'reset': () => set_back_color('#eeebe6') },
  ]
