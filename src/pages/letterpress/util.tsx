...
export const PlayerName = ({ name, profile, arrow=false, zoom=1, stats=false, icon_only=false, icon_click=undefined, handle=undefined, info_id=undefined, do_reaction=undefined, reaction=undefined }) => {
  // const base_jsx = name ? <A href={stats ? `/letterpress/stats/${name}` : `/u/${name}`}>{name}</A> : 'invite'
  ...
  ? 
    ...
    ? 
      info_id
      ? <A href={`/letterpress/game-stats/${info_id}/${name}`}>{name}</A>
      : <a onClick={e => handle.open_stats(name)}>{name}</a>
    : <A href={stats ? `/letterpress/stats/${name}` : `/u/${name}`}>{name}</A>
  : 'invite'
  const name_jsx = arrow ? <span>{'>'} {base_jsx} {'<'}</span> : base_jsx
  return <div className='letterpress-player-name grow middle-column' style={S(`
  font-size: ${zoom}em;
  `)}>
    ...
    `)} onClick={do_reaction ? e => {
      open_popup(close => {
        return <div>
          ...
          `)}>
            <span>emote!</span>
            {[...(settings?.letterpress?.reacts || '😀😎😭🫠😈')].map((reaction, i, ar) => {
...
export const PlayerNameFromInfo = ({ owner, info, display_turn:display_turn_i, played, display_owner=undefined, gameover, profiles, icon_click=undefined, handle, no_arrow=false, do_reaction=undefined }) => {
  ...
  return <PlayerName {...{
    ...
    icon_click: !do_reaction && icon_click, handle, do_reaction: do_reaction && (reaction => {
      api.post(`/letterpress/game/${info.id}/react`, { reaction })
    }),
  }} />
}
...
export const open_about = () => open_popup(close => <>
  <InfoBody id='letterpress-menu-about' className='middle-column'>
    <img src='/raw/letterpress/icon.png' width={128} style={S(`
    border: 1px solid #000;
    image-rendering: pixelated;
    `)} />
    <div><a href='https://en.wikipedia.org/wiki/Letterpress_(video_game)'>Letterpress</a> is a 2012 turn-based word game developed by Loren Brichter and published by Atebits</div>
    <HalfLine />
    {/* <div>this remake is hidden and should not be shared broadly. it was created to allow for desktop play and to add a try limit for played words</div> */}
    <div>this remake was created to allow for desktop play and to add a try limit for played words</div>
    <HalfLine />
...
export const open_howto = ({ handle, viewer }) => open_popup(close => <>
  <InfoBody id='letterpress-menu-howto' className='column'>
    <div><b>/letterpress - word strategy game</b></div>
    <HalfLine />
    <div className='column' style={S(`font-size: .8em`)}>
      ...
      >
        <div>• spell words using any tiles on the board</div>
        <div>• you keep 'unlocked' tiles</div>
        <div>• lock a tile by owning it and neighbors</div>
      </div>
      <HalfLine />
      {/* <HalfLine /> */}
