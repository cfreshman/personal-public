...
const log = named_log('letterpress menu')
...
export default ({ hf, handle, profile_map, sections }) => {
  ...
  const challenge_link = location.origin + `/letterpress/new/${viewer_game_profile?.challenge || viewer}`
  ...
  return <>
    <InfoSection className='menu-upper' labels={['new game']}>
        ...
        {local ? <div className='menu-section'>
          ...
          <div className='row gap wrap'>
            ...
            {/* <InfoButton onClick={async e => {
              ...
              store.set('letterpress-ai-speedy-ms', 700)
              handle.open(await create_game(hf, [viewer, 'speedy'], true))
            }}>medium</InfoButton>
            <InfoButton onClick={async e => {
            ...
            <InfoButton onClick={async e => {
              if (store.get('letterpress-ai-speedy-ms') < SPEEDY_DEFAULT_THINKING_TIME) {
        ...
        {online ? <div className='menu-section'>
          ...
          <div className='row gap wrap'>
            ...
            <InfoButton onClick={async () => {
              await api.post(`/letterpress/challenge`, {
      ...
      <div className='menu-section'>
        <InfoButton onClick={e => {
          set_triplepress(!triplepress)
        }}>{triplepress ? 'close' : 'triplepress'}</InfoButton>
        {triplepress ? <div className='menu-section'>
          local (human)
          <div className='row gap wrap'>
            <InfoButton onClick={async e => {
              handle.open(await create_game(hf, values(local_players).slice(0, 3), true))
