...
export default () => {
  ...
  const [seed, set_seed] = store.use('slices-3-seed', { default:818708189 })

  const [start_color, set_start_color] = store.use('slices-3-start-color', { default:"#002fff" })
  const [end_color, set_end_color] = store.use('slices-3-end-color', { default:"#00eeff" })
  const [color_long, set_color_long] = store.use('slices-3-color-long', { default:false })

  const [back_color, set_back_color] = store.use('slices-3-back-color', { default:'#eeebe6' })
  ...
  useE(noise_script, reroll, start_color, end_color, color_long, () => {
    ...
    function get_height(x, y) {
      ...
      const falloff = 1 - Math.sqrt(x * x + y * y) / (size / 2)
      // value is in [-1, 1], convert to [0, 1] to apply falloff then convert back
      return ((value + 1) / 2 * falloff) * 2 - 1
    }
