...
const { named_log, entries, values, strings, Q, range:list_range, V, rand } = window as any
const NAME = 'AOB'
...
const FruitWheel = ({ fruits, style, ...props }) => {
  ...
  `)}>
    ...
    opacity: .2;
    `)}></div> : null} */}
    {[DEFAULT_FRUITS.apple, DEFAULT_FRUITS.orange, DEFAULT_FRUITS.banana, ...fruits].map(x => {
  ...
  `)}>
    ...
    {tab === 0 ? <>
      <AOB_Section label='market' close={close}>
        ...
        : <div className='column gap'>
          {market.map(x => {
            return <button disabled={x.price > gold} onClick={async e => {
...
const FruitViewer = ({ fruit, close }) => {
  ...
  min-width: 300px;
  `)}>
    <AOB_Section label={`${fruit.name} ${fruit.emoji}`} close={close}>
      <div className='column wide gap'>
        <FruitWheel {...{ fruits:[fruit], style:`
        width: min(30em, calc(100% - 3em));
        ` }} />
      </div>
    </AOB_Section>
...
const Game = ({ data, handle }) => {
  ...
  const [view_fruit, set_view_fruit] = useS(undefined)
  useF(view_fruit, () => set_modal(view_fruit ? MODALS.VIEW : undefined))
  useF(modal, () => !modal && set_view_fruit(undefined))
  ...
  const view_fruit_objects = useM(data, () => {
    const fruits = {}
    data.plots.map(x => {
      if (x.fruit) {
      ...
      <div className='column gap'>
        <button disabled={!action} onClick={async e => {
          if (action === ACTIONS.HARVEST) {
            ...
            set_view_fruit(view_fruit_objects.find(x => x.name === fruit))
          }
        ...
        {/* <label className='action-option'><input {...fill_action(ACTIONS.VIEW)} />&nbsp;<span>
          view <select id='abo-view-fruit'>{view_fruit_objects.map(x => x.name).map(x => <option value={x}>{x}</option>)}</select> profile
        </span></label> */}
