...
const log = named_log('capture stats')
...
const special_details = {
  ...
  speedy: (rerender) => {
    const speedy_ms = store.get('capture-ai-speedy-ms') || SPEEDY_DEFAULT_THINKING_TIME
    ...
    return <>
      ...
      <InfoBadges labels={options.map(option => speedy_ms === option ? display_option(option) : {
        ...
        func: () => {
          store.set('capture-ai-speedy-ms', option)
          rerender()
        }
      }).concat([{
        reset: () => {
...
export default ({ user, hf, handle, profile_map, embedded=false }) => {
  ...
  useF(viewer, user, async () => {
    if (viewer && viewer !== user) {
      const { stats:pair_stats } = await api.get(`/capture/pair/${viewer}/${user}`)
      log({pair_stats})
  ...
  return <>
    ...
    {edit ? null : special_detail ? <>
      ...
      <InfoSection labels={[
        'stats',
        self && { share: e => {
          copy(location.origin + `/capture/stats/${user}`)
          display_status(e.target, 'copied!')
          navigator.share({ url: location.origin + `/capture/stats/${user}` })
        } }
        ]} />
      {profile ? <>
