...
const log = named_log('capture game')

const debug = {
  check: true,
}
if (dev) window['capture_game_debug'] = debug


export default ({ id, hf, handle, settings, profile_map, sections }) => {
  const [{user:viewer}] = auth.use()
  const theme = useM(settings, () => themes[settings?.capture?.theme||'default'])
  ...
  useF(gameover, () => {
    if (gameover) {
      if (info.previous) {
        message.trigger({
          text: `open previous: /capture/${info.previous}`,
          ms: 3_000,
          id: 'capture-previous', delete: 'capture-previous',
        })
      }
      if (info.rematch) {
        message.trigger({
          text: `open rematch: /capture/${info.rematch}`,
          ms: 3_000,
          id: 'capture-rematch', delete: 'capture-rematch',
        })
      } else if (member) {
        window[`capture-start-rematch-${info.id}`] = () => handle.rematch()
        message.trigger({
          text: `<a onclick="window['capture-start-rematch-${info.id}']()">start rematch!</a>`,
          ms: 3_000,
          id: 'capture-rematch', delete: 'capture-rematch',
        })
      }
      message.trigger({
        ...
        ms: 3_000,
        id: 'capture-gameover', delete: 'capture-gameover',
      })
    } else {
      message.trigger({
        delete: 'capture-gameover',
      })
    }
  })
  ...
  useF(l_board, info?.turn, state, selection, can_play, profiles, replay, replay_turn, played, play_turn, display_turn, theme, resize, () => {
          ...
          if (curr_turn) {
            ...
            if (play_turn === 0) {
              ...
              message.trigger({
                ...
                id: `capture-play-turn`, delete: `capture-play-turn`,
              })
            }
          }
          set_gameover(false)
    ...
    const capture = display_state.tiles.filter(x => x.capital)
    const under_attack_set = set(capture.flatMap(x => hf.adj(x.pos).map(pos => display_tile_map[pos.st()]).filter(x => x && x.letter).map(x => x.pos.st())))
    log({capture,under_attack_set})
  ...
  handle = {
      ...
      if (tiles.length) {
        ...
        if (played_word) {
          message.trigger({
            ...
            ms: 3_000,
            id: 'capture-already-word',
            // replace: 'capture-invalid-word',
          })
          return
        }
        ...
        const post_turn = async () => {
          ...
          if (new_info.id !== 'local' && info.owner !== new_info.owner) {
            ...
            if (next_game) {
              message.trigger({
                text: `next game vs ${user_ids(next_game).filter(x => x && x !== viewer).join(' ')||'invite'}: /capture/${next_game.id}`,
                id: `capture-next-game`, delete: `capture-next-game`,
                ms: 3_000,
              })
            }
          }
    ...
    //     id: `capture-ai-thinking`, delete: `capture-ai-thinking`,
    //   })
    //   defer(async () => {
    ...
    rematch: async () => {
      ...
      if (info.id === 'local') {
        ...
        handle.load_list()
        message.trigger({
          delete: `capture-play-turn capture-ai-thinking capture-gameover capture-rematch capture-previous`,
        })
      } else {
        log('rematch', await api.post(`/capture/game/${info.id}/rematched/${rematch_info.id}`))
  ...
  useSocket({
    on: {
      'capture:update': updated_id => {
        log('capture:update', updated_id)
        if (id === updated_id) {
          handle.load()
          // message.trigger({
          //   text: 'your turn!',
  ...
  useF(_info, member, is_local, async () => {
    ...
    log({invite})
    if (invite) {
      const {info} = await api.post(`/capture/game/${id}/join`)
      set_info(info)
    }
  })

  const show_chat = useM(info, member, is_local, settings, () => member && !is_local && info?.chat && (settings?.capture?.chat??true))
  const [chat_open, set_chat_open] = useS(false)
  const [chat_unread, set_chat_unread] = useS(0)
  useE(info, chat_open, () => {
    if (can_replay) {
      const ls_words = QQ('#chat-container :is(.capital-word, .capture-word)').reverse()
  ...
  useStyle(profiles, `
  #chat-container :is(.capital-word, .capture-word) {
    position: relative;
    z-index: 1;
    padding: .1em .33em !important;
    border-radius: .25em !important;
  }
  #chat-container :is(.capital-word, .capture-word)::after {
    content: "";
    position: absolute;
  ...
  #chat-container :is(.capital-word, .capture-word).p${i} {
    border: .05em solid ${profile.color} !important;
    border: max(1px, .05em) solid #000 !important;
  }
  #chat-container :is(.capital-word, .capture-word).p${i}::after {
  ...
  return <>
    ...
    <div id='game-controls' className='row wide gap'>
      ...
      : <>
        <InfoButton onClick={e => {
          open_popup(close => <>
            <InfoBody id='capture-game-options' className='middle-column'>
              ...
              ?
                ...
                ? <InfoButton onClick={() => {
                  close()
                  open_popup(close => <>
                    <InfoBody id='capture-game-resign' className='middle-column'>
    ...
    <div id='board-container' className='grow middle-column' ref={board_root}>
      ...
      ? <div id='chat-container' className={`chat-visible-${chat_open}`}>
        <Chat {...{ hash:`capture:${id}`, reading:chat_open, setUnread:set_chat_unread, flipped:info.p1===viewer, special:'capture-word capital-word' }} />
      </div> 
      : <div id='chat-container' className={`chat-visible-${chat_open}`}>
        ...
        `)}>
          {info.turns.slice().reverse().map((turn, i) => {
            ...
            return <div className={`capture-word p${turn.owner} ${skip?'capture-skip':''}`} style={S(`
