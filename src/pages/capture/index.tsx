...
import { COOKIES_CAPTURE, create_game, fetch_game, fetch_profile, play_turn, update_game, user_ids } from './data'
import { useSocket } from 'src/lib/socket'
import { get_selection, is_ai } from './ai'
import { dict, load_lang } from '../../lib/dict'
...
export default () => {
  ...
  const [profile_map, set_profile_map] = store.use(COOKIES_CAPTURE.PROFILES, { default:{} })
  ...
  useF(modes.new, page_id, async () => {
    if (modes.new && page_id) {
      ...
      const { user, id } = await api.get(`/capture/challenge/${page_id}`)
  ...
  const handle = {
    ...
    load_list: async () => {
      ...
      const { list=[] } = viewer ? await api.get('/capture') : {}
      log({list})
      set_list_info([local, ...list])
    },
    load_info: async (id) => {
      const {info} = await api.get(`/capture/game/${id}/info`)
          ...
          defer(async () => {
            try {
              ...
              const selection = await get_selection(info, state, hf, (progress) => trigger_for_game({
                ...
                id: `capture-ai-thinking`, replace: `capture-ai-thinking`,
              ...
              handle.load_list()
              trigger_for_game({
                text: `${active_player} played ${new_info.turns.at(-1).word.toUpperCase()} in capture: /capture/local`,
                ms: 5_000,
  ...
  // useF(settings?.capture?.music, list_name => list_name && track_play(list_name, {do_shuffle:false}))
  useStyle(`
  #game-modal {
    width: max-content;
    height: max-content;
      ...
      {modes.new ? <NiceModal on_close={e => {
        ...
        {challenge_link ? <>
          ...
          {!challenge_link.user ? <>
            ...
            }}>{`/capture/${challenge_link.id}`}</InfoButton>
          </> : <>
