...
const log = named_log('lettergo stats')
...
const special_details = {
  ...
  speedy: (rerender) => {
    const speedy_ms = store.get('lettergo-ai-speedy-ms') || SPEEDY_DEFAULT_THINKING_TIME
    ...
    return <>
      speedy could play better than anyone if they took a little longer
      <div>&nbsp;</div>
      ...
      <InfoBadges labels={options.map(option => speedy_ms === option ? display_option(option) : {
        ...
        func: () => {
          store.set('lettergo-ai-speedy-ms', option)
          rerender()
        }
      }).concat([{
        reset: () => {
...
export default ({ user, hf, handle, profile_map, embedded=false }) => {
  ...
  useF(viewer, user, async () => {
    if (viewer && viewer !== user) {
      const { stats:pair_stats } = await api.get(`/lettergo/pair/${viewer}/${user}`)
      log({pair_stats})
  ...
  return <>
    ...
    {edit ? null : special_detail ? <>
      ...
      <InfoSection labels={[
        'stats',
        self && { share: e => {
          copy(location.origin + `/lettergo/stats/${user}`)
          display_status(e.target, 'copied!')
          navigator.share({ url: location.origin + `/lettergo/stats/${user}` })
        } }
        ]} />
      {profile ? <>
