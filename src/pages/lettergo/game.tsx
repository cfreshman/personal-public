...
import { Info, State, clone_info, clone_state, construct_state, create_game, fetch_game, update_game, play_turn as do_play_turn, play_state as do_play_state, construct_tile_map, profile_colors, is_local_player, profile_icons, user_ids, next_owner, BOARD_SIZE } from './data'
import { is_valid_word, rand_alpha } from '../../lib/dict'
...
const { named_log, node, svg_node, V, range, set, rand, Q, QQ, on, list, devices, strings, defer, values, lists, from } = window as any
const log = named_log('lettergo game')

const debug = {
  check: false,
}
if (dev) window['lettergo_game_debug'] = debug


export default ({ id, hf, handle, settings, profile_map, sections }) => {
  const [{user:viewer}] = auth.use()
  const theme = useM(settings, () => themes[settings?.lettergo?.theme||'default'])
  ...
  useF(gameover, () => {
    if (gameover) {
      if (info.previous) {
        message.trigger({
          text: `open previous: /lettergo/${info.previous}`,
          ms: 3_000,
          id: 'lettergo-previous', delete: 'lettergo-previous',
        })
      }
      if (info.rematch) {
        message.trigger({
          text: `open rematch: /lettergo/${info.rematch}`,
          ms: 3_000,
          id: 'lettergo-rematch', delete: 'lettergo-rematch',
        })
      } else if (member) {
        window[`lettergo-start-rematch-${info.id}`] = () => handle.rematch()
        message.trigger({
          text: `<a onclick="window['lettergo-start-rematch-${info.id}']()">start rematch!</a>`,
          ms: 3_000,
          id: 'lettergo-rematch', delete: 'lettergo-rematch',
        })
      }
      message.trigger({
        ...
        ms: 3_000,
        id: 'lettergo-gameover', delete: 'lettergo-gameover',
      })
    } else {
      message.trigger({
        delete: 'lettergo-gameover',
      })
    }
  })
  ...
  useF(l_board, info?.turn, state, selection, can_play, profiles, replay, replay_turn, played, play_turn, display_turn, theme, resize, () => {
    ...
    <style>
      #board {
        width: ${BOARD_SIZE.W}em;
        height: ${BOARD_SIZE.H}em;
        border: 1px solid var(--id-color-text);
      }
      #board .tile {
        width: ${100 / BOARD_SIZE.W}%;
        height: ${100 / BOARD_SIZE.H}%;
          ...
          if (curr_turn) {
            ...
            if (play_turn === 0) {
              ...
              message.trigger({
                ...
                id: `lettergo-play-turn`, delete: `lettergo-play-turn`,
              })
            }
          }
          set_gameover(false)
    ...
    const lettergo = display_state.tiles.filter(x => x.capital)
    const under_attack_set = set(lettergo.flatMap(x => hf.adj(x.pos).map(pos => display_tile_map[pos.st()]).filter(x => x && x.letter).map(x => x.pos.st())))
    log({lettergo,under_attack_set})
    ...
    display_state.tiles.map((tile, i) => {
      ...
      const actual_color = !color ? theme.tile.off : base_tile.locked ? color : layerBackground(theme.tile.off, `${color}80`)
      inner_html += `
      <div id="group_${i}" class="tile ${selected?'selected':''}" style="
  ...
  handle = {
    ...
    play: async (tiles=selection) => {
      if (tiles.length) {
        ...
        const played_word = info.words.find(other => other.startsWith(word)) || (info.tried_words||[]).includes(word)
        if (played_word) {
          message.trigger({
            text: `${played_word.toUpperCase()} has already been played`,
            ms: 3_000,
            id: 'lettergo-already-word',
            // replace: 'lettergo-invalid-word',
          })
          return
        }
        ...
        const post_turn = async () => {
          ...
          if (new_info.id !== 'local' && info.owner !== new_info.owner) {
            ...
            if (next_game) {
              message.trigger({
                text: `next game vs ${user_ids(next_game).filter(x => x && x !== viewer).join(' ')||'invite'}: /lettergo/${next_game.id}`,
                id: `lettergo-next-game`, delete: `lettergo-next-game`,
                ms: 3_000,
              })
            }
          }
    ...
    //     id: `lettergo-ai-thinking`, delete: `lettergo-ai-thinking`,
    //   })
    //   defer(async () => {
    ...
    //     message.trigger({
    //       delete: `lettergo-ai-thinking`,
    //     })
    //   }, 500)
    // },
    rematch: async () => {
      const display_state = construct_state(state)
      const user_to_n_tiles = from(user_ids(info).map((user, i) => [user, display_state.tiles.filter(tile => tile.owner === i).length]))
      const users = user_ids(info).sort((a, b) => user_to_n_tiles[a] - user_to_n_tiles[b])
      message.trigger({
        text: 'creating rematch',
        id: 'lettergo-rematch-create',
      })
      const { info:rematch_info, state:rematch_state } = await create_game(hf, users, info.id === 'local')
      message.trigger({
        delete: 'lettergo-rematch-create',
      })
      if (info.id === 'local') {
        ...
        handle.load_list()
        message.trigger({
          delete: `lettergo-play-turn lettergo-ai-thinking lettergo-gameover lettergo-rematch lettergo-previous`,
        })
      } else {
        log('rematch', await api.post(`/lettergo/game/${info.id}/rematched/${rematch_info.id}`))
  ...
  useSocket({
    on: {
      'lettergo:update': updated_id => {
        log('lettergo:update', updated_id)
        if (id === updated_id) {
          handle.load()
          // message.trigger({
          //   text: 'your turn!',
  ...
  useF(_info, member, is_local, async () => {
    ...
    log({invite})
    if (invite) {
      const {info} = await api.post(`/lettergo/game/${id}/join`)
      set_info(info)
    }
  })

  const show_chat = useM(info, member, is_local, settings, () => member && !is_local && info?.chat && (settings?.lettergo?.chat??true))
  const [chat_open, set_chat_open] = useS(false)
  const [chat_unread, set_chat_unread] = useS(0)
  useE(info, chat_open, () => {
    if (can_replay) {
      const ls_words = QQ('#chat-container :is(.capital-word, .lettergo-word)').reverse()
  ...
  useStyle(profiles, `
  #chat-container :is(.capital-word, .lettergo-word) {
    position: relative;
    z-index: 1;
    padding: .1em .33em !important;
    border-radius: .25em !important;
  }
  #chat-container :is(.capital-word, .lettergo-word)::after {
    content: "";
    position: absolute;
  ...
  #chat-container :is(.capital-word, .lettergo-word).p${i} {
    border: .05em solid ${profile.color} !important;
    border: max(1px, .05em) solid #000 !important;
  }
  #chat-container :is(.capital-word, .lettergo-word).p${i}::after {
  ...
  return <>
    {user_ids(info).length === 2 ? <>
      <div id='game-players' className='row wide gap'>
        ...
        {some_tile_count
        ? <div id='game-players-vs'>{tile_counts.join(' - ')}</div>
        : <span id='game-players-vs'>vs</span>}
        {player_name_for_i(1)}
      </div>
    </> : <>
      <div id='game-players' className='row wide gap'>
        {range(user_ids(info).length).map(i => <div className='game-player-count-and-name center-column'>
          <span id='game-players-vs'>{tile_counts[i]}</span>
    ...
    <div id='game-word' className='middle-row'>
      ...
      ? <div id='game-controls-word'>
        <Reorderable elements={selection.map(tile => {
          ...
          const actual_color = !color ? theme.tile.off : tile.locked ? color : layerBackground(theme.tile.off, `${color}80`)
          return <span className='game-controls-tile' style={S(`
          background: ${actual_color};
          color: var(--id-color-text);
          `)} onClick={e => {
            setTimeout(() => {
    ...
    <div id='game-controls' className='row wide gap'>
      ...
      : <>
        <InfoButton onClick={e => {
          open_popup(close => <>
            <InfoBody id='lettergo-game-options' className='middle-column'>
              ...
              ?
                ...
                ? <InfoButton onClick={() => {
                  close()
                  open_popup(close => <>
                    <InfoBody id='lettergo-game-resign' className='middle-column'>
    ...
    <div id='board-container' className='grow middle-column' ref={board_root}>
      ...
      ? <div id='chat-container' className={`chat-visible-${chat_open}`}>
        <Chat {...{ hash:`lettergo:${id}`, reading:chat_open, setUnread:set_chat_unread, flipped:info.p1===viewer, special:'lettergo-word capital-word' }} />
      </div> 
      : <div id='chat-container' className={`chat-visible-${chat_open}`}>
        ...
        `)}>
          {info.turns.slice().reverse().map((turn, i) => {
            ...
            return <div className={`lettergo-word p${turn.owner} ${skip?'lettergo-skip':''}`} style={S(`
