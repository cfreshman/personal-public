...
import { COOKIES_LETTERGO, create_game, fetch_game, fetch_profile, play_turn, update_game, user_ids } from './data'
...
export default () => {
  ...
  const [profile_map, set_profile_map] = store.use(COOKIES_LETTERGO.PROFILES, { default:{} })
  ...
  useF(modes.new, page_id, async () => {
    if (modes.new && page_id) {
      ...
      const { user, id } = await api.get(`/lettergo/challenge/${page_id}`)
  ...
  const handle = {
    ...
    load_list: async () => {
      ...
      const { list=[] } = viewer ? await api.get('/lettergo') : {}
      log({list})
      set_list_info([local, ...list])
    },
    load_info: async (id) => {
      const {info} = await api.get(`/lettergo/game/${id}/info`)
          ...
          defer(async () => {
            try {
              ...
              const selection = await get_selection(info, state, hf, (progress) => trigger_for_game({
                ...
                id: `lettergo-ai-thinking`, replace: `lettergo-ai-thinking`,
              ...
              handle.load_list()
              trigger_for_game({
                text: `${active_player} played ${new_info.turns.at(-1).word.toUpperCase()} in lettergo: /lettergo/local`,
                ms: 5_000,
  ...
  // useF(settings?.lettergo?.music, list_name => list_name && track_play(list_name, {do_shuffle:false}))
  useStyle(`
  #game-modal {
    width: max-content;
    height: max-content;
      ...
      {modes.new ? <NiceModal on_close={e => {
        ...
        {challenge_link ? <>
          ...
          {!challenge_link.user ? <>
            ...
            }}>{`/lettergo/${challenge_link.id}`}</InfoButton>
          </> : <>
