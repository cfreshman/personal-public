...
const log = named_log('lettergo menu')
...
export default ({ hf, handle, profile_map, sections }) => {
  ...
  const challenge_link = location.origin + `/lettergo/new/${viewer_game_profile?.challenge || viewer}`
        ...
        {local ? <div className='menu-section'>
          ...
          <div className='row gap wrap'>
            ...
            {/* <InfoButton onClick={async e => {
              ...
              store.set('lettergo-ai-speedy-ms', 700)
              handle.open(await create_game(hf, [viewer, 'speedy'], true))
            }}>medium</InfoButton>
            <InfoButton onClick={async e => {
              store.set('lettergo-ai-speedy-ms', 3_000)
              handle.open(await create_game(hf, [viewer, 'speedy'], true))
            }}>hard</InfoButton> */}
            {/* <InfoButton onClick={async e => {
            ...
            <InfoButton onClick={async e => {
              if (store.get('lettergo-ai-speedy-ms') < SPEEDY_DEFAULT_THINKING_TIME) {
                store.set('lettergo-ai-speedy-ms', SPEEDY_DEFAULT_THINKING_TIME)
              }
              message.trigger({
                text: `open speedy's profile to adjust their thinking time (current: ${datetimes.durations.pretty(store.get('lettergo-ai-speedy-ms')||SPEEDY_DEFAULT_THINKING_TIME)})`,
                ms: 3_000,
              })
              handle.open(await create_game(hf, [viewer, 'speedy'], true))
            }}>speedy</InfoButton>
        ...
        {online ? <div className='menu-section'>
          ...
          <div className='row gap wrap'>
            ...
            <InfoButton onClick={async () => {
              await api.post(`/lettergo/challenge`, {
