...
const log = named_log('lettergo data')
...
export const fetch_profile = async (name) => {
  ...
  const { profile } = await api.get(`/lettergo/profile/${name}`)
  return profile
}
...
export const COOKIES_LETTERGO = {
  PROFILES: 'lettergo-profiles',
  INFO: 'lettergo-info',
  STATE: 'lettergo-state',
}
...
export const fetch_game = async (id: string, hf: any): Promise<{ info: Info, state: State }> => {
  if (id === 'local') {
    const info = store.get(COOKIES_LETTERGO.INFO)
    const state = store.get(COOKIES_LETTERGO.STATE)
    if (!info) return create_game(hf, ['bottom', 'top'], true)
    return { info:hydrate_info(info), state:hydrate_state(state) }
  } else {
    const { info=undefined, state=undefined } = await api.get(`/lettergo/game/${id}`)
    return { info:hydrate_info(info), state:hydrate_state(state) }
  }
}
const tutorial_messages = [
  'find some words! tap to select tiles\n<b>start with LETTERGO in the top left</b>',
...
export const update_game = async (info: Info, state: State) => {
  ...
  if (id === 'local') {
    ...
    store.set(COOKIES_LETTERGO.INFO, info)
    store.set(COOKIES_LETTERGO.STATE, state)

    message.trigger({ delete:'tutorial-hint' })
    if (info.turn % 2 === 0 && store.get('lettergo-tutorial-run')) {
      const index = store.get('lettergo-tutorial-index')
      // alert(`tutorial ${index}`)
      if (index >= tutorial_messages.length) {
        store.set('lettergo-tutorial-run', undefined)
      } else {
        message.trigger({
          text: tutorial_messages[index],
          id: 'tutorial-hint',
        })
        store.set('lettergo-tutorial-index', index + 1)
      }
    }
  } else {
    await api.post(`/lettergo/game/${id}`, { info, state })
  }
}
...
const new_state = (hf, info): State => {
  const is_tutorial = store.get('lettergo-tutorial-start')
  store.set('lettergo-tutorial-start', undefined)
  store.set('lettergo-tutorial-index', 0)
  store.set('lettergo-tutorial-run', is_tutorial)
...
export const create_game = async (hf: any, users: string[], local:boolean=false): Promise<{ info: Info, state: State }> => {
  ...
  if (local) {
    ...
    const { info:server_info=undefined, state:server_state=undefined } = await api.post(`/lettergo/game`, { info, state })
