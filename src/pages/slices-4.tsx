...
export default () => {
  ...
  const [seed, set_seed] = store.use('slices-4-seed', { default:rand.i(1_000_000_000) })

  const [start_color, set_start_color] = store.use('slices-4-start-color', { default:undefined })
  const [end_color, set_end_color] = store.use('slices-4-end-color', { default:undefined })
  const [color_long, set_color_long] = store.use('slices-4-color-long', { default:false })

  const [back_color, set_back_color] = store.use('slices-4-back-color', { default:'#eeebe6' })
  ...
  useE(noise_script, reroll, start_color, end_color, color_long, () => {
    ...
    function get_border_radius(x, y, z=0) {
      ...
      return (
        .1 * noise.simplex3(a_x / 32 + 10_000_000 + seed, a_y / 32 + 10_000_000 + seed, z / 32) + 
        .4 * noise.simplex3(a_x / 8 + 20_000_000 + seed, a_y / 8 + 20_000_000 + seed, z / 8) + 
        .4 * noise.simplex3(a_x / 2 + 30_000_000 + seed, a_y / 2 + 30_000_000 + seed, z / 2) + 
      ...
      const color = gradient[i]
      {
        ...
        const shape_alphamap = (() => {
          ...
          for (let angle = 0; angle < Math.PI * 2; angle += Math.PI / 32) {
            const border = get_border_radius(Math.cos(angle) * SZ / 2, Math.sin(angle) * SZ / 2, 2 * i / N_SLICES)
            const height = (border * SZ / 4) * (1 - .125 * (i / (N_SLICES - 1)))
            points.push({
              x: Math.cos(angle) * height + SZ / 2,
              y: Math.sin(angle) * height + SZ / 2,
            })
          }
          // draw shapes
          ctx.fillStyle = 'white'
