...
import { useEventListener, useF, useM, useR, useRerender, useS, useStyle, useTimed } from 'src/lib/hooks'
...
import { COOKIES_CAPITALS, create_game, fetch_game, fetch_profile, play_turn, update_game } from './data'
...
export default () => {
  ...
  const [profile_map, set_profile_map] = store.use(COOKIES_CAPITALS.PROFILES, { default:{} })
  ...
  useF(modes.new, page_id, async () => {
    if (modes.new && page_id) {
      ...
      const { user, id } = await api.get(`/capitals/challenge/${page_id}`)
  ...
  const handle = {
    ...
    load_list: async () => {
      ...
      const { list=[] } = viewer ? await api.get('/capitals') : {}
      log({list})
      set_list_info([local, ...list])
    },
    load_info: async (id) => {
      const {info} = await api.get(`/capitals/game/${id}/info`)
  ...
  const hecks_reload = useRerender()
  useCachedScript('/lib/2/hecks/script.js', hecks_reload)
  const hecks = useM(hecks_reload, () => window['hecks'])
  const hf = useM(hecks, () => hecks && new hecks.Field(1, hecks.Field.Orientation.TOP_FLAT))

  useSocket({
    on: {
      'capitals:update': id => {
        log('capitals:update', id)
        // handle.load_info(id)
        handle.load_list()
      }
    }
        ...
        if (is_ai(active_player)) {
          ...
          defer(async () => {
            try {
              ...
              handle.load_list()
              trigger_for_game({
                text: `${active_player} played ${new_info.turns.at(-1).word.toUpperCase()} in lettercomb: /lettercomb/local`,
                ms: 5_000,
  ...
  usePageSettings({
    // professional: true,
    background: '#fff',
    icon: '/raw/capitals/icon.png',
  })
  // useF(settings?.capitals?.music, list_name => list_name && track_play(list_name, {do_shuffle:false}))
  useStyle(`
  #game-modal {
    width: max-content;
    height: max-content;
  ...
  return <Style>
    ...
    `)}>
      {modes.game
      ? hecks ? <Game {...{ id:page_id, hf, handle, settings, profile_map, sections }} /> : <div>loading!</div>
      : modes.stats
      ...
      {modes.new ? <NiceModal on_close={e => {
        ...
        {challenge_link ? <>
          ...
          {!challenge_link.user ? <>
            ...
            }}>{`/lettercomb/${challenge_link.id}`}</InfoButton>
          </> : <>
