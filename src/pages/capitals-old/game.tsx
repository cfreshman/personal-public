...
import { Info, State, clone_info, clone_state, construct_state, create_game, fetch_game, update_game, play_turn as do_play_turn, play_state as do_play_state, construct_tile_map, profile_colors, is_local_player, profile_icons } from './data'
...
export default ({ id, hf, handle, settings, profile_map, sections }) => {
  ...
  useF(gameover, () => {
    if (gameover) {
      if (info.previous) {
        message.trigger({
          text: `open previous: /capitals/${info.previous}`,
          ms: 3_000,
      ...
      if (info.rematch) {
        message.trigger({
          text: `open rematch: /capitals/${info.rematch}`,
          ms: 3_000,
  ...
  const member = useM(viewer, info, () => info.id === 'local' || (info.id && [info.p0, info.p1].includes(viewer)))
  ...
  useF(info, () => {
    handle.load_profiles([info.p0, info.p1], true)
  })
  useF(info, profile_map, theme, () => {
    const new_profiles = range(2).map(i => ({
  ...
  useF(svg, info?.turn, state, selection, can_play, profiles, replay, replay_turn, played, play_turn, double_play, display_turn, () => {
    ...
    const turn_owner = !display_turn ? 0 : 1 - info.turns.at(display_turn-(played?1:2))?.owner||0
    ...
    display_state.tiles.map((tile, i) => {
      ...
      const color = selected ? 'var(--id-color)' : tile_played ? theme.tile.tentative : [profiles[0].color, profiles[1].color][tile.owner] || (tile.letter ? theme.tile.letter : theme.tile.off)
  ...
  handle = {
    ...
    load: async (load_id=id) => {
      try {
        ...
        set_info(new_info)
        if (new_info.turn !== info.turn || [info.p0, info.p1].join('-') !== [new_info.p0, new_info.p1].join('-')) {
          set_state(new_state)
        }
      } catch (e) {
        set_info({} as any)
      ...
      if (tiles.length) {
        ...
        const post_turn = async () => {
          ...
          if (new_info.id !== 'local' && info.owner !== new_info.owner) {
            ...
            if (next_game) {
              message.trigger({
                text: `next game vs ${[next_game.p0, next_game.p1].find(x => x && x !== viewer)||'invite'}: /capitals/${next_game.id}`,
    ...
    // },
    rematch: async () => {
      const users = info.status === 0 ? [info.p1, info.p0].map(x => x === 'top' ? 'bottom' : x === 'bottom' ? 'top' : x) : [info.p0, info.p1]
      message.trigger({
  ...
  useF(viewer, _info, async () => {
    ...
    const invite = [_info.p0, _info.p1].some(x => !x)
    log({invite})
    if (invite) {
  ...
  const show_chat = useM(info, is_local, settings, () => !is_local && info?.chat && (settings?.capitals?.chat??true))
  const [chat_open, set_chat_open] = useS(false)
  const [chat_unread, set_chat_unread] = useS(0)
  useE(info, chat_open, () => {
    if (can_replay) {
      const ls_words = QQ('#chat-container :is(.capital-word, .capitals-word)').reverse()
  ...
  useStyle(profiles, `
  #chat-container :is(.capital-word, .capitals-word) {
    position: relative;
    z-index: 1;
    padding: .1em .33em !important;
    border-radius: .25em !important;
  }
  #chat-container :is(.capital-word, .capitals-word)::after {
    content: "";
    position: absolute;
  ...
  const display_owner = useM(info, display_turn, word, last_turn_object, () => word.length ? info.owner : last_turn_object?.owner || 1 - info.owner)
  ...
  return <>
    ...
    <div id='game-players' className='row wide gap'>
      <PlayerNameFromInfo {...{ owner:1, info, display_turn, played, gameover, profiles, icon_click:icon_click_for(info.p1), handle }} do_reaction={!is_local && !word && display_turn && display_turn === info.turn && last_turn_object?.owner === 1 && info.p1 === viewer} />
      <span id='game-players-vs'>vs</span>
      <PlayerNameFromInfo {...{ owner:0, info, display_turn, played, gameover, profiles, icon_click:icon_click_for(info.p0), handle }} do_reaction={!is_local && !word && display_turn && display_turn === info.turn && last_turn_object?.owner === 0 && info.p0 === viewer} />
    </div>
    <div id='game-controls' className='row wide gap'>
      ...
      {/* {word ? <div id='game-controls-word'>{word}</div> : null} */}
      ...
      : <>
        <InfoButton onClick={e => {
          open_popup(close => <>
            <InfoBody id='capitals-game-options' className='middle-column'>
              ...
              {[info.p0, info.p1].map(user => !user || is_local_player(user) || is_ai(user) ? null : <InfoButton onClick={e => {
                close()
        ...
        {member ? <InfoButton onClick={e => set_chat_open(!chat_open)}>{chat_open ? 'close' : chat_unread ? `${chat_unread} unread` : show_chat ? 'chat' : 'turns'}</InfoButton> : null}
      </>}
    </div>
    <div id='board-container' className='grow middle-column' ref={hex_root}>
      {info.chat && show_chat
      ? <div id='chat-container' className={`chat-visible-${chat_open}`}>
        <Chat {...{ hash:`capitals:${id}`, reading:chat_open, setUnread:set_chat_unread, flipped:info.p1===viewer, special:'capitals-word capital-word' }} />
      </div> 
      : <div id='chat-container' className={`chat-visible-${chat_open}`}>
