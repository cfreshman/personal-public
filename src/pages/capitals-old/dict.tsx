...
export const set_lang = (lang: string) => {
    console.debug(`CAPITALS LANG`, lang)
    dict.set({
        lang,
        loaded: false,
        words: new Set<string>(),
    ...
    return load_lang(lang).then(value => {
        ...
        dict.set(value)
        console.debug('CAPITALS LANG LOADED', dict.lang)
    })
}
...
export const load_lang = (lang: string='english'): Promise<any> => Promise.resolve(Object.load(dicts, lang, () => {
    ...
    const fetch_dict = () => {
        ...
        .then(res => {
            console.debug(`CAPITALS DICT FETCHED`, lang)
        ...
        .then(word_list => {
            console.debug(`CAPITALS DICT DESERIALIZED`, lang)
...
const freqsCalc = {}
target && dict.add(() => {
    console.debug('CAPITALS FREQ', target, dict)
    ...
    console.debug('CAPITALS FREQ', target, freqsCalc)
    if (Object.keys(freqsCalc).length === 2) {
        ...
        freqsCalc['alpha'] = letters
        {
            ...
            console.log('CAPITALS FREQ', target, freqsCalc)
        }
    }
})