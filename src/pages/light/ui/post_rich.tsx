...
export const PostRich = ({ href, handle, single=false, level=1, no_new=false, userpage=false }) => {
  ...
  `)}>
    ...
    : mode === 'image' ? <img src={full_href} style={S(`width:100%; max-height:30em; object-fit:contain; background: var(--id-color-label); ${single?'':'aspect-ratio: 2/1;'}`)} />
    ...
    : mode === 'video' ? <video src={full_href} controls playsInline style={S(`width:100%; max-height:30em; object-fit:contain; background: var(--id-color-label); ${single?'':'aspect-ratio: 2/1;'}`)} />
