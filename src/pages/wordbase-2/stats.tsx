  ...
  speedy: (rerender) => {
    ...
    return <>
      ...
      <InfoBadges labels={options.map(option => speedy_ms === option ? display_option(option) : {
        ...
        func: () => {
          store.set('letterpress-ai-speedy-ms', option)
          rerender()
        }
      }).concat([{
        reset: () => {
...
export default ({ user, hf, handle, profile_map, embedded=false }) => {
  ...
  useF(viewer, user, async () => {
    if (viewer && viewer !== user) {
      const { stats:pair_stats } = await api.get(`/letterpress/pair/${viewer}/${user}`)
      log({pair_stats})
  ...
  return <>
    <InfoSection labels={[
      ...
      edit && { save: async () => {
        const { profile } = await api.post(`/letterpress/profile`, edit_data.current)
    ...
    {edit ? null : special_detail ? <>
      ...
      <InfoSection labels={[
        'stats',
        self && { share: e => {
          copy(location.origin + `/letterpress/stats/${user}`)
