...
import { Info, State, clone_info, clone_state, construct_state, create_game, fetch_game, update_game, play_turn as do_play_turn, play_state as do_play_state, construct_tile_map, profile_colors, is_local_player, profile_icons, user_ids, next_owner } from './data'
...
const log = named_log('letterpress game')

const debug = {
  check: true,
}
if (dev) window['letterpress_game_debug'] = debug
...
export default ({ id, hf, handle, settings, profile_map, sections }) => {
  ...
  useF(gameover, () => {
    if (gameover) {
      if (info.previous) {
        message.trigger({
          text: `open previous: /letterpress/${info.previous}`,
          ms: 3_000,
          id: 'letterpress-previous', delete: 'letterpress-previous',
        })
      }
      if (info.rematch) {
        message.trigger({
          text: `open rematch: /letterpress/${info.rematch}`,
          ms: 3_000,
          id: 'letterpress-rematch', delete: 'letterpress-rematch',
        })
      } else if (member) {
        window[`letterpress-start-rematch-${info.id}`] = () => handle.rematch()
        message.trigger({
          text: `<a onclick="window['letterpress-start-rematch-${info.id}']()">start rematch!</a>`,
          ms: 3_000,
          id: 'letterpress-rematch', delete: 'letterpress-rematch',
        })
      }
      message.trigger({
        ...
        ms: 3_000,
        id: 'letterpress-gameover', delete: 'letterpress-gameover',
      })
    } else {
      message.trigger({
        delete: 'letterpress-gameover',
      })
    }
  })
  ...
  useEventListener(window, 'resize deviceorientation', () => resize())
  ...
  useF(l_board, info?.turn, state, selection, can_play, profiles, replay, replay_turn, played, play_turn, display_turn, theme, resize, () => {
          ...
          if (curr_turn) {
            ...
            if (play_turn === 0) {
              ...
              message.trigger({
                ...
                id: `letterpress-play-turn`, delete: `letterpress-play-turn`,
              })
            }
          }
          set_gameover(false)
    ...
    const letterpress = display_state.tiles.filter(x => x.capital)
    const under_attack_set = set(letterpress.flatMap(x => hf.adj(x.pos).map(pos => display_tile_map[pos.st()]).filter(x => x && x.letter).map(x => x.pos.st())))
    log({letterpress,under_attack_set})
  ...
  handle = {
      ...
      if (tiles.length) {
        ...
        if (played_word) {
          message.trigger({
            ...
            ms: 3_000,
            id: 'letterpress-already-word',
            // replace: 'letterpress-invalid-word',
          })
          return
        }
        ...
        const post_turn = async () => {
          ...
          if (new_info.id !== 'local' && info.owner !== new_info.owner) {
            ...
            if (next_game) {
              message.trigger({
                text: `next game vs ${user_ids(next_game).filter(x => x && x !== viewer).join(' ')||'invite'}: /letterpress/${next_game.id}`,
                id: `letterpress-next-game`, delete: `letterpress-next-game`,
                ms: 3_000,
              })
            }
          }
    ...
    rematch: async () => {
      ...
      if (info.id === 'local') {
        ...
        log('rematch', await api.post(`/letterpress/game/${info.id}/rematched/${rematch_info.id}`))
  ...
  useSocket({
    on: {
      'letterpress:update': updated_id => {
        log('letterpress:update', updated_id)
        if (id === updated_id) {
          handle.load()
          // message.trigger({
          //   text: 'your turn!',
  ...
  useF(_info, member, is_local, async () => {
    ...
    log({invite})
    if (invite) {
      const {info} = await api.post(`/letterpress/game/${id}/join`)
      set_info(info)
    }
  })

  const show_chat = useM(info, member, is_local, settings, () => member && !is_local && info?.chat && (settings?.letterpress?.chat??true))
  const [chat_open, set_chat_open] = useS(false)
  const [chat_unread, set_chat_unread] = useS(0)
  useE(info, chat_open, () => {
    if (can_replay) {
      const ls_words = QQ('#chat-container :is(.capital-word, .letterpress-word)').reverse()
  ...
  useStyle(profiles, `
  #chat-container :is(.capital-word, .letterpress-word) {
    position: relative;
    z-index: 1;
    padding: .1em .33em !important;
    border-radius: .25em !important;
  }
  #chat-container :is(.capital-word, .letterpress-word)::after {
    content: "";
    position: absolute;
  ...
  #chat-container :is(.capital-word, .letterpress-word).p${i} {
    border: .05em solid ${profile.color} !important;
    border: max(1px, .05em) solid #000 !important;
  }
  #chat-container :is(.capital-word, .letterpress-word).p${i}::after {
  ...
  return <>
    ...
    <div id='game-controls' className='row wide gap'>
      ...
      : <>
        <InfoButton onClick={e => {
          open_popup(close => <>
            <InfoBody id='letterpress-game-options' className='middle-column'>
              ...
              ?
                ...
                ? <InfoButton onClick={() => {
                  close()
                  open_popup(close => <>
                    <InfoBody id='letterpress-game-resign' className='middle-column'>
    ...
    <div id='board-container' className='grow middle-column' ref={board_root}>
      ...
      ? <div id='chat-container' className={`chat-visible-${chat_open}`}>
        <Chat {...{ hash:`letterpress:${id}`, reading:chat_open, setUnread:set_chat_unread, flipped:info.p1===viewer, special:'letterpress-word capital-word' }} />
      </div> 
      : <div id='chat-container' className={`chat-visible-${chat_open}`}>
        ...
        `)}>
          {info.turns.slice().reverse().map((turn, i) => {
            ...
            return <div className={`letterpress-word p${turn.owner} ${skip?'letterpress-skip':''}`} style={S(`
