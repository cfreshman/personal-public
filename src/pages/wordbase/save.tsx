...
export class Save {
    ...
    static pair(save: Save) {
        // return competitive pair for save (right now, simply the same game)
        ...
        while (bombFlips.length) {
            ...
            deep.tiles().forEach(t => {
                ...
                if (Tile.has(flips, t)) {
                    ...
                    shockMs = Pos.dist(bomb, t) * 60
                }
                t.shocks = (t.shocks || []).concat([[type, Math.round(currMs + shockMs), Pos.new(bomb)]])
...
export class Info {
    ...
    constructor({
        ...
        // console.trace('WORDBASE new info', this)
    }
    static local(difficulty=0, settings: GameSettings=GameSettings.Normal()) {
        if (settings) {
            settings = JSON.parse(JSON.stringify(settings))
            delete settings.options.theme
        }
    ...
    static getRemainingTime(info: Info): {
        ...
        const responsibleForTimeout = auth.user === info[info.turn % 2 ? 'p2' : 'p1'] || info.id === 'local'
