...
import { useEventListener, useF, useInterval, useR, useRerender, useStyle } from '../lib/hooks';
import { useAuth, usePageSettings, useHashState, usePathState } from '../lib/hooks_ext';
...
const resize = () => {
  ...
  const containerWidth = Number(style.width.slice(0, -2)) - 1;
  const containerHeight = Number(style.height.slice(0, -2)) - 1;
...
export default () => {
  const [{ user:viewer, expand }, set_auth] = auth.use()
  const [hash, setHash] = usePathState()
  ...
  useF(viewer, () => {
    if (viewer) {
      api.get(`/profile/${viewer}`).then(({ profile }) => {
        setProfile(profile)
        if (viewer !== 'cyrus'
            && hash !== viewer && !profile?.friends.includes(hash)) {
          setHash('')
        }
      })
    } else {
  ...
  useF(() => {
    if (devices.is_mobile && expand) {
      set_auth({ ...auth.get(), expand:false })
    }
  })
  ...
  `)
  return <Style id='page-graffiti'>
    <div className="selection">
      <div className="pages">
        <div className={''+(hash === '')} onClick={() => setHash('')}>public</div>
        {viewer
        ?
        // [viewer, ...(array(20, i => randAlphanum(7)))].map(user =>
        [viewer, ...(profile?.friends || [])].map(user =>
