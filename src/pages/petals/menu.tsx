...
const log = named_log('petals menu')
...
const MenuGameItem = ({ viewer, profile_map=undefined, info, handle }) => {
  ...
  const action = word ? `played ${word.toUpperCase()}` : (info.status > -1 && info.status !== turn.owner) ? 'resigned' : 'skipped'
...
export default ({ hf, handle, profile_map, sections }) => {
  ...
  const challenge_link = location.origin + `/petals/new/${viewer_game_profile?.challenge || viewer}`
          ...
          {null&&<>
            ...
            <div className='row gap wrap'>
              ...
              {/* <InfoButton onClick={async e => {
                ...
                store.set('petals-ai-speedy-ms', 700)
                handle.open(await create_game(hf, [viewer, 'speedy'], true))
              }}>medium</InfoButton>
              <InfoButton onClick={async e => {
                store.set('petals-ai-speedy-ms', 3_000)
                handle.open(await create_game(hf, [viewer, 'speedy'], true))
              }}>hard</InfoButton> */}
              {/* <InfoButton onClick={async e => {
              ...
              <InfoButton onClick={async e => {
                if (store.get('petals-ai-speedy-ms') < SPEEDY_DEFAULT_THINKING_TIME) {
                  store.set('petals-ai-speedy-ms', SPEEDY_DEFAULT_THINKING_TIME)
                }
                message.trigger({
                  text: `open speedy's profile to adjust their thinking time (current: ${datetimes.durations.pretty(store.get('petals-ai-speedy-ms')||SPEEDY_DEFAULT_THINKING_TIME)})`,
                  ms: 3_000,
                })
                handle.open(await create_game(hf, [viewer, 'speedy'], true))
              }}>speedy</InfoButton>
        ...
        {online ? <div className='menu-section'>
          ...
          <div className='row gap wrap'>
            ...
            <InfoButton onClick={async () => {
              await api.post(`/petals/challenge`, {
