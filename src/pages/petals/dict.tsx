...
export const set_lang = (lang: string) => {
    console.debug(`PETALS LANG`, lang)
    dict.set({
        lang,
        loaded: false,
        words: new Set<string>(),
    ...
    return load_lang(lang).then(value => {
        ...
        dict.set(value)
        console.debug('PETALS LANG LOADED', dict.lang)
    })
}
...
export const load_lang = (lang: string='english'): Promise<any> => Promise.resolve(Object.load(dicts, lang, () => {
    ...
    const fetch_dict = () => {
        ...
        .then(res => {
            console.debug(`PETALS DICT FETCHED`, lang)
        ...
        .then(word_list => {
            console.debug(`PETALS DICT DESERIALIZED`, lang)
            ...
            dicts[lang] = _dict
            try {
                store.set(`petals-dict`, { ..._dict, words: [..._dict.words] })
            } catch {} // too large
            return _dict
        })
    ...
    const existing = store.get(`petals-dict`)
...
const freqsCalc = {}
target && dict.add(() => {
    console.debug('PETALS FREQ', target, dict)
    ...
    console.debug('PETALS FREQ', target, freqsCalc)
    if (Object.keys(freqsCalc).length === 2) {
        ...
        freqsCalc['alpha'] = letters
        {
            ...
            console.log('PETALS FREQ', target, freqsCalc)
        }
    }
})