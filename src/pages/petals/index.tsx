...
import { COOKIES_PETALS, create_game, fetch_game, fetch_profile, play_turn, update_game, user_ids } from './data'
...
const { named_log, list, set, defer, keys, devices } = window as any
const log = named_log('petals')
...
export default () => {
  ...
  const [profile_map, set_profile_map] = store.use(COOKIES_PETALS.PROFILES, { default:{} })
  ...
  useF(modes.new, page_id, async () => {
    if (modes.new && page_id) {
      ...
      const { user, id } = await api.get(`/petals/challenge/${page_id}`)
  ...
  const handle = {
    ...
    load_list: async () => {
      ...
      const { list=[] } = viewer ? await api.get('/petals') : {}
      log({list})
      set_list_info([local, ...list])
    },
    load_info: async (id) => {
      const {info} = await api.get(`/petals/game/${id}/info`)
          ...
          defer(async () => {
            try {
              ...
              const selection = await get_selection(info, state, hf, (progress) => trigger_for_game({
                ...
                id: `petals-ai-thinking`, replace: `petals-ai-thinking`,
              ...
              handle.load_list()
              trigger_for_game({
                text: `${active_player} played ${new_info.turns.at(-1).word.toUpperCase()} in petals: /petals/local`,
                ms: 5_000,
  ...
  usePageSettings({
    // professional: true,
    background: '#fff',
    icon: '/raw/images/icon-petals.png',
    // expand: !devices.is_mobile,
  })
  // useF(settings?.petals?.music, list_name => list_name && track_play(list_name, {do_shuffle:false}))
  useStyle(`
  #game-modal {
    width: max-content;
    height: max-content;
      ...
      {modes.new ? <NiceModal on_close={e => {
        ...
        {challenge_link ? <>
          ...
          {!challenge_link.user ? <>
            ...
            }}>{`/petals/${challenge_link.id}`}</InfoButton>
          </> : <>
