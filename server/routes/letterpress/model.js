...
const names = {
    game_profile: 'letterpress_profile',
        // user: string-user
        // t: number
        // ids: string-id[]
        ...
        // color: string-hex
        // icon: string
    game_info: 'letterpress_info',
        // id: string-id
        // info state
        // multipress: boolean
    game_state: 'letterpress_state',
        // id: string-id
        // game state
    game_invite: 'letterpress_invite',
        // id: string-id
        // user: string
        // n_users: number
    game_spectate: 'letterpress_spectate'
        // id: string-id
        // users: string-user[]
}
...
db.queueInit(async () => {
    ...
    const daily_stats = async () => {
        ...
        console.debug('letterpress daily stats', {active_info_n})
        await M_counter.setlock('site', 'letterpress', 'active', active_info_n)
    }
    daily_stats()
    setTimeout(daily_stats, day_ms)
}, 5_000)

const chat_id = (info) => `letterpress:${info.id}`
...
async function profile(viewer, user) {
    console.debug('[letterpress:profile] get', {viewer,user})
    ...
    profile.stats.active_games = (await M_counter.get('site', 'letterpress', 'active', true)).value
    profile.stats.global_letters = (await M_counter.get('site', 'letterpress', 'letters', true)).value
...
async function profile_set(viewer, data) {
    console.debug('[letterpress:set_profile]', {viewer,data})
...
async function infos(viewer) {
    console.debug('[letterpress:infos] get', {viewer})
...
async function game_new(viewer, data) {
    ...
    notify.send(unique_others, 'letterpress', `${viewer} started a new letterpress game`, `freshman.dev/letterpress/${id}`)
    io.send(unique_others, 'message', {
        text: `${viewer} started a new letterpress game: /letterpress/${id}`,
        ms: 5_000,
        id: 'letterpress-new', delete: 'letterpress-new',
        to: `/letterpress/${id}`,
    })
    unique_others.map(other => siteChat(other, `${viewer} started a new letterpress game: /letterpress/${id}`))
...
async function game_turn(viewer, id, data) {
    ...
    notify.send(gameover ? unique_others : [next_user], 'letterpress', `${viewer} ${action} in letterpress`, `freshman.dev/letterpress/${id}`)
    io.send(unique_others, 'message', {
        text: `${viewer} ${action} in letterpress: /letterpress/${id}`,
        ms: 5_000,
        id: 'letterpress-turn', delete: 'letterpress-turn',
        to: `/letterpress/${id}`,
    })
    if (gameover) unique_others.map(other => siteChat(other, `${viewer} ${action} in letterpress: /letterpress/${id}`))
    ...
    unique_ids(users).map(async (user, i) => {
        ...
        if (!info.multipress) {
            if (info.owner !== i) {
                ...
                let {value:total_letters} = await M_counter.get('site', 'letterpress', 'letters', true)
                total_letters += word.length
                await M_counter.setlock('site', 'letterpress', 'letters', total_letters)
            }
    ...
    await chat.sendChat(viewer, chat_id(info), [{
        ...
        meta: {
            silent: true,
            classes: `letterpress-word ${word?'':'letterpress-skip'} p${saved_info.owner}`,
            dedupe: info.turn,
        }
    }])
    io.send(unique_ids([...unique_users, ...(await _spectators(id))]), 'letterpress:update', id)
...
async function game_react(viewer, id, body) {
    ...
    await C.game_info().updateOne({ id }, { $set:info }, { upsert:true })
    io.send(unique_ids([...users, ...(await _spectators(id))]), 'letterpress:update', id)
...
async function game_join(viewer, id) {
    ...
    notify.send(unique_others, 'letterpress', `${viewer} joined your letterpress game`, `freshman.dev/letterpress/${id}`)
    io.send(unique_others, 'message', {
        text: `${viewer} joined your letterpress game: /letterpress/${id}`,
        ms: 5_000,
        id: 'letterpress-joined', delete: 'letterpress-joined',
        to: `/letterpress/${id}`,
    })
    unique_others.map(other => siteChat(other, `${viewer} joined your letterpress game: /letterpress/${id}`))
    io.send(unique_ids([...unique_others, ...(await _spectators(id))]), 'letterpress:update', id)
...
async function pair_stats(viewer, user_1, user_2) {
    console.debug('[letterpress:pair_stats] get', {viewer, user_1, user_2})
...
async function set_challenge_hash(viewer, { on=undefined, randomize=undefined }={}) {
    ...
    if (on !== undefined || randomize !== undefined) {
        ...
        console.info('[letterpress:challenge_hash]', {challenge,challenge_on}, '->', {new_challenge,new_challenge_on})
    }
