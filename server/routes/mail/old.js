...
let ons = []
const is_mail_ready = () => !!gmail
const on_mail_ready = (fn) => {
  if (is_mail_ready()) {
    fn()
  } else {
    ons.push(fn)
...
readSecret(CREDS_SECRET).then(creds => {
  console.debug('read mail creds', creds)
  authorize(creds, (auth) => {
    console.debug('mail auth', auth)
    gmail = google.gmail({version: 'v1', auth})
    console.debug('should be authorized', gmail)
    ons.forEach(fn => fn())
  })
}).catch(console.log)
...
function authorize(credentials, callback) {
  ...
  readSecret(TOKEN_SECRET).then(token => {
    console.debug('read mail token', token)
...
function buildRequest(subject, domain, to, message, extraHeaders=[]) {
  ...
  console.debug(gmail.context._options.auth)
  return {
    auth: gmail.context._options.auth,
    userId: 'me',
    resource: {
