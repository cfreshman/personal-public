...
const names = {
    game_profile: 'petals_profile',
        ...
        // color: string-hex
        // icon: string
    game_info: 'petals_info',
        // id: string-id
        // info state
        // multipress: boolean
    game_state: 'petals_state',
        // id: string-id
        // game state
    game_invite: 'petals_invite',
        // id: string-id
        // user: string
        // n_users: number
    game_spectate: 'petals_spectate'
        // id: string-id
        // users: string-user[]
}
...
db.queueInit(async () => {
    ...
    const daily_stats = async () => {
        ...
        console.debug('petals daily stats', {active_info_n})
        await M_counter.setlock('site', 'petals', 'active', active_info_n)
    }
    daily_stats()
    setTimeout(daily_stats, day_ms)
}, 5_000)

const chat_id = (info) => `petals:${info.id}`
...
async function profile(viewer, user) {
    console.debug('[petals:profile] get', {viewer,user})
    ...
    profile.stats.active_games = (await M_counter.get('site', 'petals', 'active', true)).value
    profile.stats.global_letters = (await M_counter.get('site', 'petals', 'letters', true)).value
...
async function profile_set(viewer, data) {
    console.debug('[petals:set_profile]', {viewer,data})
...
async function infos(viewer) {
    console.debug('[petals:infos] get', {viewer})
...
async function game_new(viewer, data) {
    ...
    notify.send(unique_others, 'petals', `${viewer} started a new petals game`, `freshman.dev/petals/${id}`)
    io.send(unique_others, 'message', {
        text: `${viewer} started a new petals game: /petals/${id}`,
        ms: 5_000,
        id: 'petals-new', delete: 'petals-new',
        to: `/petals/${id}`,
    })
    unique_others.map(other => siteChat(other, `${viewer} started a new petals game: /petals/${id}`))
...
async function game_turn(viewer, id, data) {
    const { info:proto_info, state:proto_state } = data
    const { id:info_id, turn, owner, turns, scores, status, tries, words } = proto_info
    ...
    const { word, owner: turn_owner } = last_turn
    const state = {
        id,
        tiles,
        deltas,
    ...
    const action = word ? `${gameover ? 'won with' : 'played'} ${word.toUpperCase()}` : gameover ? turn_owner === info.status ? 'won with a skip' : 'resigned' : 'skipped'
    const next_user = info[`p${info.owner}`]
    notify.send(gameover ? unique_others : [next_user], 'petals', `${viewer} ${action} in petals`, `freshman.dev/petals/${id}`)
    io.send(unique_others, 'message', {
        text: `${viewer} ${action} in petals: /petals/${id}`,
        ms: 5_000,
        id: 'petals-turn', delete: 'petals-turn',
        to: `/petals/${id}`,
    })
    if (gameover) unique_others.map(other => siteChat(other, `${viewer} ${action} in petals: /petals/${id}`))
    ...
    unique_ids(users).map(async (user, i) => {
        ...
        if (!info.multipress) {
            if (info.owner !== i) {
                ...
                let {value:total_letters} = await M_counter.get('site', 'petals', 'letters', true)
                total_letters += word.length
                await M_counter.setlock('site', 'petals', 'letters', total_letters)
            }
    ...
    await chat.sendChat(viewer, chat_id(info), [{
        ...
        meta: {
            silent: true,
            classes: `petals-word ${word?'':'petals-skip'} p${saved_info.owner}`,
            dedupe: info.turn,
        }
    }])
    io.send(unique_ids([...unique_users, ...(await _spectators(id))]), 'petals:update', id)
...
async function game_react(viewer, id, body) {
    ...
    await C.game_info().updateOne({ id }, { $set:info }, { upsert:true })
    io.send(unique_ids([...users, ...(await _spectators(id))]), 'petals:update', id)
...
async function game_join(viewer, id) {
    ...
    notify.send(unique_others, 'petals', `${viewer} joined your petals game`, `freshman.dev/petals/${id}`)
    io.send(unique_others, 'message', {
        text: `${viewer} joined your petals game: /petals/${id}`,
        ms: 5_000,
        id: 'petals-joined', delete: 'petals-joined',
        to: `/petals/${id}`,
    })
    unique_others.map(other => siteChat(other, `${viewer} joined your petals game: /petals/${id}`))
    io.send(unique_ids([...unique_others, ...(await _spectators(id))]), 'petals:update', id)
...
async function pair_stats(viewer, user_1, user_2) {
    console.debug('[petals:pair_stats] get', {viewer, user_1, user_2})
...
async function set_challenge_hash(viewer, { on=undefined, randomize=undefined }={}) {
    ...
    if (on !== undefined || randomize !== undefined) {
        ...
        console.info('[petals:challenge_hash]', {challenge,challenge_on}, '->', {new_challenge,new_challenge_on})
    }
