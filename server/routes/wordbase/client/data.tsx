...
// migrate old local info/save - keep till 11/1
if (store.get('info')) {
...
export const local = trigger.implicitOf(store.persist<{ info: Info, save: Save }>('wordbase-local', {
    ...
    save: value => value && [value.info, value.save?.serialize()],
}))
...
export async function fetchGame(gameId: string): Promise<{
    ...
    return new Promise(resolve => {
        ...
        if (gameId === local.info.id) {
            ...
            const user = gameId.split('new/')[1]
            // console.log('[wordbase:new]', user)
            const save = Save.new()
            api.post(`/wordbase/i/new/${user}`, { state: save.serialize() })
            .then(data => {
...
export function createLocal(difficulty?: number, settings?: GameSettings): Promise<{info: Info, save: Save}> {
