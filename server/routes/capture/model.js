...
const names = {
    game_profile: 'capture_profile',
        ...
        // color: string-hex
        // icon: string
    game_info: 'capture_info',
        // id: string-id
        // info state
        // multipress: boolean
    game_state: 'capture_state',
        // id: string-id
        // game state
    game_invite: 'capture_invite',
        // id: string-id
        // user: string
        // n_users: number
    game_spectate: 'capture_spectate'
        // id: string-id
        // users: string-user[]
}
...
db.queueInit(async () => {
    ...
    const daily_stats = async () => {
        ...
        console.debug('capture daily stats', {active_info_n})
        await M_counter.setlock('site', 'capture', 'active', active_info_n)
    }
    daily_stats()
    setTimeout(daily_stats, day_ms)
}, 5_000)

const chat_id = (info) => `capture:${info.id}`
...
async function profile(viewer, user) {
    console.debug('[capture:profile] get', {viewer,user})
    ...
    profile.stats.active_games = (await M_counter.get('site', 'capture', 'active', true)).value
    profile.stats.global_letters = (await M_counter.get('site', 'capture', 'letters', true)).value
...
async function profile_set(viewer, data) {
    console.debug('[capture:set_profile]', {viewer,data})
...
async function infos(viewer) {
    console.debug('[capture:infos] get', {viewer})
...
async function game_new(viewer, data) {
    ...
    notify.send(unique_others, 'capture', `${viewer} started a new capture game`, `freshman.dev/capture/${id}`)
    io.send(unique_others, 'message', {
        text: `${viewer} started a new capture game: /capture/${id}`,
        ms: 5_000,
        id: 'capture-new', delete: 'capture-new',
        to: `/capture/${id}`,
    })
    unique_others.map(other => siteChat(other, `${viewer} started a new capture game: /capture/${id}`))
...
async function game_turn(viewer, id, data) {
    ...
    notify.send(gameover ? unique_others : [next_user], 'capture', `${viewer} ${action} in capture`, `freshman.dev/capture/${id}`)
    io.send(unique_others, 'message', {
        text: `${viewer} ${action} in capture: /capture/${id}`,
        ms: 5_000,
        id: 'capture-turn', delete: 'capture-turn',
        to: `/capture/${id}`,
    })
    if (gameover) unique_others.map(other => siteChat(other, `${viewer} ${action} in capture: /capture/${id}`))
    ...
    unique_ids(users).map(async (user, i) => {
        ...
        if (!info.multipress) {
            if (info.owner !== i) {
                ...
                let {value:total_letters} = await M_counter.get('site', 'capture', 'letters', true)
                total_letters += word.length
                await M_counter.setlock('site', 'capture', 'letters', total_letters)
            }
    ...
    await chat.sendChat(viewer, chat_id(info), [{
        ...
        meta: {
            silent: true,
            classes: `capture-word ${word?'':'capture-skip'} p${saved_info.owner}`,
            dedupe: info.turn,
        }
    }])
    io.send(unique_ids([...unique_users, ...(await _spectators(id))]), 'capture:update', id)
...
async function game_react(viewer, id, body) {
    ...
    await C.game_info().updateOne({ id }, { $set:info }, { upsert:true })
    io.send(unique_ids([...users, ...(await _spectators(id))]), 'capture:update', id)
...
async function game_join(viewer, id) {
    ...
    notify.send(unique_others, 'capture', `${viewer} joined your capture game`, `freshman.dev/capture/${id}`)
    io.send(unique_others, 'message', {
        text: `${viewer} joined your capture game: /capture/${id}`,
        ms: 5_000,
        id: 'capture-joined', delete: 'capture-joined',
        to: `/capture/${id}`,
    })
    unique_others.map(other => siteChat(other, `${viewer} joined your capture game: /capture/${id}`))
    io.send(unique_ids([...unique_others, ...(await _spectators(id))]), 'capture:update', id)
...
async function pair_stats(viewer, user_1, user_2) {
    console.debug('[capture:pair_stats] get', {viewer, user_1, user_2})
...
async function set_challenge_hash(viewer, { on=undefined, randomize=undefined }={}) {
    ...
    if (on !== undefined || randomize !== undefined) {
        ...
        console.info('[capture:challenge_hash]', {challenge,challenge_on}, '->', {new_challenge,new_challenge_on})
    }
